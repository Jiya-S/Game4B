class SpaceShooter extends Phaser.Scene {
    ...
    create() {
        ...
        this.player = this.physics.add.sprite(this.screenWidth / 2, this.screenHeight - 50, 'player1_walk', 0);
        this.player.setCollideWorldBounds(true);
        // Scale down since the actual character is small in the 256x256 frame
        this.player.setScale(0.35); // Tweak this value as you trim the sprite
        // --- DEBUG: Draw a rectangle around the player for hitbox visualization ---
        this.playerDebugRect = this.add.rectangle(this.player.x, this.player.y, 40, 40, 0xff0000, 0.2);
        this.playerDebugRect.setStrokeStyle(2, 0xff0000);
        this.playerDebugRect.setDepth(10);
        ...
        this.livesText = this.add.text(this.screenWidth - 160, 16, 'Lives: ' + this.lives, {
        ...
        // --- BACKGROUND MUSIC ---
        this.bgMusic = this.sound.add('spaceshootermusic', { loop: true, volume: 0.5 });
        this.bgMusic.play();
    }
    ...
    update() {
        ...
        // --- DEBUG: Keep debug rectangle on player ---
        if (this.playerDebugRect) {
            this.playerDebugRect.setPosition(this.player.x, this.player.y);
        }
    }
    ...
    fire() {
        ...
        this.sound.play('laserShoot', { volume: 0.7 });
    }
    ...
    hitPlayer(player, enemy) {
        ...
        if (this.health <= 0) {
            ...
            if (this.lives > 0) {
                this.health = 100;
                this.player.setPosition(this.screenWidth / 2, this.screenHeight - 50);
                this.player.setActive(true).setVisible(true);
                this.gameOver = false;
                this.add.text(this.screenWidth / 2, this.screenHeight / 2, 'You lost a life!\nLives left: ' + this.lives + '\nPress SPACE to continue', {
                    fontSize: '32px', fill: '#fff', align: 'center'
                }).setOrigin(0.5);
                this.input.keyboard.once('keydown-SPACE', () => {
                    this.children.getAll().forEach(child => { if (child.text && child.text.startsWith('You lost a life!')) child.destroy(); });
                });
            } else {
                this.gameOver = true;
                this.updateHighScore();
                this.add.text(this.screenWidth / 2, this.screenHeight / 2, 'Game Over\nScore: ' + this.score + '\nHigh Score: ' + this.highScore + '\nReturning to Platformer...', {
                    fontSize: '32px', fill: '#fff', align: 'center'
                }).setOrigin(0.5);
